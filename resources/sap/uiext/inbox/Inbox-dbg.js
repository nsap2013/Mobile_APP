        ...
        for ( var i = 0; i < entitiesLength; i++) {
          if (i === 0) {
            ...
            // }).setWidth("20%").setLabel(new sap.ui.commons.Label({ //FIXME: Column  is not visible  
        					...
        					tooltip : this._oBundle.getText("INBOX_TASK_TITLE"),
        					design : sap.ui.commons.LabelDesign.Bold
        				})).setTemplate(oFirstColHorLay).setSortProperty(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.properties[i])
        				...
        				oTable.addColumn(new sap.ui.table.Column({
        					...
        					text : this._oBundle.getText(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.propertiesLabel[i]),
        					tooltip : this._oBundle.getText("INBOX_FILTER_CREATION_DATE"),
        					design : sap.ui.commons.LabelDesign.Bold
        				})).setTemplate(
        				...
        				oTable.addColumn(new sap.ui.table.Column({
        					...
        					text : this._oBundle.getText(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.propertiesLabel[i]),
        					tooltip : this._oBundle.getText("INBOX_DUE_DATE"),
        					design : sap.ui.commons.LabelDesign.Bold
        				})).setTemplate(
        						new sap.ui.commons.TextView({
        							text : {
      				...
      				var taskStatusTxt = new sap.ui.commons.TextView();
      				taskStatusTxt.bindProperty("text", "Status", function(value) {
      				...
      				oTable.addColumn(new sap.ui.table.Column({
      					...
      					text : this._oBundle.getText(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.propertiesLabel[i]),
      					tooltip : this._oBundle.getText("INBOX_STATUS"),
      					design : sap.ui.commons.LabelDesign.Bold
      				})).setTemplate(taskStatusTxt).setSortProperty(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.properties[i])
      				...
      				oTable.addColumn(new sap.ui.table.Column({
      					...
      					text : this._oBundle.getText(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.propertiesLabel[i]),
      					tooltip : this._oBundle.getText("INBOX_PRIORITY"),
      					design : sap.ui.commons.LabelDesign.Bold
...
//			         tooltip : this._oBundle.getText("INBOX_MSGBAR_CLOSE_TOOLTIP")
			...
			//taskName.addStyleClass("sapUiExtInboxNoUnderline");
	 	    taskName.bindProperty("text","TaskTitle");
 	    ...
 	    statusText.bindProperty("text","Status", function(value) {
		   ...
		   var taskInitiatorName = new sap.ui.commons.TextView(that.getId() + '--' + "taskInitiatorName",{design : sap.ui.commons.TextViewDesign.Small});
	 	   ...
	 	   var claimActionCell = new sap.ui.commons.layout.MatrixLayoutCell({
	 	    ...
	 	    var claimActionLink = new sap.ui.commons.Link(that.getId() + '--' + "rrViewClaimAction",{tooltip:that._oBundle.getText("INBOX_ACTION_BUTTON_CLAIM")});
	 	   ...
	 	   claimActionLink.bindProperty("visible","SupportsClaim",function(value){
	 	    ...
	 	    var releaseActionLink = new sap.ui.commons.Link(that.getId() + '--' + "rrViewReleaseAction",{tooltip:that._oBundle.getText("INBOX_ACTION_BUTTON_RELEASE")});
...
sap.uiext.inbox.Inbox.prototype.bindTasks = function(sPath, oFilter, oSorter) {
  ...
  //TODO: the handle Task Category enablement is before the settting of the flags. Check whats happening here.
    ...
    if(oModl.sServiceUrl){
    	this.oTcmMetadata.setServiceMetadata(oModl.getServiceMetadata());
    }
...
sap.uiext.inbox.Inbox.prototype.switchViews = function(oEvent, oInbox) {
    ...
    var taskDefinitionFilter = false, aSelectedKeys;
							...
							if(buttonClickedID.indexOf(that.getId() + '--' + 'customAttributesSegBtn')!= -1){
								//if(this.data("previousSelection") !== buttonClickedID){
									...
									//Show/Hide tooltip fix
										...
										var sSelectedTaskInstance = that.getModel().getProperty("InstanceID",oEvent.getSource().getParent().getBindingContext()); 
										var sSapOrigin = that.getModel().getProperty("SAP__Origin",oEvent.getSource().getParent().getBindingContext());
										var aCustomAttributeDefinitionArray = that._getCustomAttributeMetaData(sSelectedTaskDefinition,sSapOrigin);
										if(aCustomAttributeDefinitionArray && aCustomAttributeDefinitionArray.length>0){
											var aCustomAttributeValueArray = that._getCustomAttributeData(sSelectedTaskInstance,sSapOrigin);
											lastRowCell.addContent(that._createCustomAttributesRRViewMatrixLayout(aCustomAttributeDefinitionArray,aCustomAttributeValueArray,rowNumber)); 
...
sap.uiext.inbox.Inbox.prototype._createTaskLink = function(){
	...
	oTaskLink.bindProperty("text", taskTitleProperty);
